...
function preProcess(font, references, limit) {
	...
	function makeVariance(valueDefault, valueWghtMax) {
		...
		let valueHeavy = roundTo(valueWghtMax);
		return valueFactory.create(valueLight, [[masterWghtMax, valueHeavy - valueLight]]);
	}
	...
	function checkSingleGlyph(glyph) {
						...
						for (let idxP2 = 0; idxP2 < contour2.length; idxP2++) {
							...
							if (
								...
								if (contour2.length.isBetween(10, 13)) {
									...
									let strokeAvgH = (hStrokeH + vStrokeTopH) / 2;
									let vStrokeDeltaH = vStrokeTopH - strokeAvgH;
									contour2[pVNew0I] = Ot.Glyph.Point.create(
										makeVariance(originLight(pVNew0.x), originHeavy(pVNew0.x) - vStrokeDeltaH),
										makeVariance(originLight(pVNew0.y), originHeavy(pVNew0.y)),
										0
									);
									contour2[pVNew1I] = Ot.Glyph.Point.create(
										makeVariance(originLight(pVNew1.x), originHeavy(pVNew1.x) - vStrokeDeltaH),
		...
		for (let [idxC1, contour] of oldContours.entries()) {
			...
			if (name === "uni3F17" && idxC1 === 5) console.log(newContour);
			...
			if (newContour.length > 12) {
				for (let idxP = 0; idxP < newContour.length; idxP++) {
					...
					if (p1.kind === 0 && p2.kind === 1 && p3.kind === 2 && p4.kind === 0 && p5.kind === 1 && p6.kind === 2 && p7.kind === 0 && p8.kind === 1 && p9.kind === 2 && p10.kind === 0) {
						...
						if (name === "uni3F3E") console.log(corner1Angle, corner2Angle);
						if (
							hookHeight > 10 &&
		...
		for (let idxC1 = 0; idxC1 < oldContours.length; idxC1++) {
			...
			for (let idxP1 = 0; idxP1 < contour.length; idxP1++) {
				...
				if (
						...
						if (endAngles1H[0] < endAngles1H[1]) {
							...
							if (inside2.length > 0) {
								for (const idxC2 of inside2) {
									...
									if (polygonHeavy.length > 1) {
										for (let i = 1; i < polygonHeavy.length; i++) {
											if (geometric.lineIntersectsPolygon([p1H, n2H], polygonHeavy[i])) fail = true;
										}
									}
								}
							} else {
							...
							if (!fail) {
								...
								n2H = closestPointOnLine(p1H, [p2H, p3H]);
								if (inside2.length > 0) {
									for (const idxC2 of inside2) {
										if (idxC2 === idxC1 || polyGlyphLight[idxC2] === undefined) continue;
										let polygonHeavy = polyGlyphHeavy[idxC2];
										...
										if (polygonHeavy.length > 1) {
											for (let i = 1; i < polygonHeavy.length; i++) {
												if (geometric.lineIntersectsPolygon([m1H, n2H], polygonHeavy[i])) fail2 = true;
											}
										}
									}
								}
							...
							if (inside1.length > 0) {
								for (const idxC2 of inside1) {
									...
									if (polygonHeavy.length > 1) {
										for (let i = 1; i < polygonHeavy.length; i++) {
											if (geometric.lineIntersectsPolygon([n1H, p2H], polygonHeavy[i])) fail = true;
										}
									}
								}
							} else {
							...
							if (!fail) {
								...
								n1H = closestPointOnLine(p2H, [p0H, p1H]);
								if (inside1.length > 0) {
									for (const idxC2 of inside1) {
										if (idxC2 === idxC1 || polyGlyphLight[idxC2] === undefined) continue;
										let polygonHeavy = polyGlyphHeavy[idxC2];
										...
										if (polygonHeavy.length > 1) {
											for (let i = 1; i < polygonHeavy.length; i++) {
												if (geometric.lineIntersectsPolygon([n1H, m2H], polygonHeavy[i])) fail2 = true;
											}
										}
									}
								}
						...
						if (endAngles2H[0] < endAngles2H[1]) {
							...
							if (inside6.length > 0) {
								for (const idxC2 of inside6) {
									...
									if (polygonHeavy.length > 1) {
										for (let i = 1; i < polygonHeavy.length; i++) {
											if (geometric.lineIntersectsPolygon([p5H, n6H], polygonHeavy[i])) fail = true;
										}
									}
								}
							} else {
							...
							if (!fail) {
								...
								n6H = closestPointOnLine(p5H, [p6H, p7H]);
								if (inside6.length > 0) {
									for (const idxC2 of inside6) {
										if (idxC2 === idxC1 || polyGlyphLight[idxC2] === undefined) continue;
										let polygonHeavy = polyGlyphHeavy[idxC2];
										...
										if (polygonHeavy.length > 1) {
											for (let i = 1; i < polygonHeavy.length; i++) {
												if (geometric.lineIntersectsPolygon([m5H, n6H], polygonHeavy[i])) fail2 = true;
											}
										}
									}
								}
							...
							if (inside5.length > 0) {
								for (const idxC2 of inside5) {
									...
									if (polygonHeavy.length > 1) {
										for (let i = 1; i < polygonHeavy.length; i++) {
											if (geometric.lineIntersectsPolygon([n5H, p6H], polygonHeavy[i])) fail = true;
										}
									}
								}
							} else {
							...
							if (!fail) {
								...
								n5H = closestPointOnLine(p6H, [p4H, p5H]);
								if (inside5.length > 0) {
									for (const idxC2 of inside5) {
										if (idxC2 === idxC1 || polyGlyphLight[idxC2] === undefined) continue;
										let polygonHeavy = polyGlyphHeavy[idxC2];
										...
										if (polygonHeavy.length > 1) {
											for (let i = 1; i < polygonHeavy.length; i++) {
												if (geometric.lineIntersectsPolygon([n5H, m6H], polygonHeavy[i])) fail2 = true;
											}
										}
									}
								}
					...
					// if (JSON.stringify(oldContours[idxC1][0]) !== JSON.stringify(oldContours[idxC1][last])) {
					// 	oldContours[idxC1][0] = oldContours[idxC1][last];
					// }
					// oldContours[idxC1] = [...oldContours[idxC1], oldContours[idxC1][0]];
					if (name === "uni3598" && idxC1 === 6) console.log(oldContours[idxC1]);
					// 
					skipContours.push(idxC1);
					break;
				}
