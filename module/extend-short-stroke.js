		...
		for (const contour of oldContours) {
			...
			for (let idx = 0; idx < contour.length; idx++) {
				...
				if (
					...
					const topRightIdx = circularIndex(contour, idx + 1);
					const topLeftIdx = circularIndex(contour, idx + 2);
					const bottomLeftIdx = circularIndex(contour, idx - 1);
							...
							if (
								...
								if (
									...
									newContour[bottomRightIdx] = {
										x: makeVariance(
											Ot.Var.Ops.originOf(ctr[ctrIdx].x) - 1,
											Ot.Var.Ops.evaluate(ctr[ctrIdx].x, instanceShsWghtMax) - 2
										),
								...
								if (
									...
									newContour[bottomLeftIdx] = {
										x: makeVariance(
											Ot.Var.Ops.originOf(verticalTopLeft.x) + 1,
											Ot.Var.Ops.evaluate(verticalTopLeft.x, instanceShsWghtMax) + 2
										),
				...
				if (
					...
					const bottomRightIdx = circularIndex(contour, idx + 1);
					const topRightIdx = circularIndex(contour, idx + 2);
					const topLeftIdx = circularIndex(contour, idx - 1);
							...
							if (
								...
								if (
									...
									newContour[bottomLeftIdx] = {
										...
										y: makeVariance(
											Ot.Var.Ops.originOf(horizontalBottomLeft.y) + 7,
											Ot.Var.Ops.evaluate(horizontalBottomLeft.y, instanceShsWghtMax) + 30
										),
										kind: 0,
									};
									newContour[bottomRightIdx] = {
				...
				if (
					...
					const topLeftIdx = circularIndex(contour, idx + 1);
					const bottomLeftIdx = circularIndex(contour, idx + 2);
					const bottomRightIdx = circularIndex(contour, idx - 1);
					...
					// 	abs(Ot.Var.Ops.originOf(verticalTopLeft.y) - Ot.Var.Ops.originOf(circularArray(contour, idx + 3).y)) > 30 &&
					// 	abs(Ot.Var.Ops.originOf(verticalBottomLeft.x) - Ot.Var.Ops.originOf(circularArray(contour, idx + 3).x)) < 10
					// ) {
					// 	const deltaM0 = Ot.Var.Ops.originOf(verticalTopLeft.y) - Ot.Var.Ops.originOf(verticalBottomLeft.y);
					// 	const deltaM1 = Ot.Var.Ops.evaluate(verticalTopLeft.y, instanceShsWghtMax) - Ot.Var.Ops.evaluate(verticalBottomLeft.y, instanceShsWghtMax);
					...
					// 		y: makeVariance(
					// 			Ot.Var.Ops.originOf(verticalBottomLeft.y) - diffM0,
					// 			Ot.Var.Ops.evaluate(verticalBottomLeft.y, instanceShsWghtMax) - diffM1
					// 			),
					...
					// 	abs(Ot.Var.Ops.originOf(verticalTopRight.y) - Ot.Var.Ops.originOf(verticalBottomRight.y)) < 30 &&
					// 	abs(Ot.Var.Ops.originOf(verticalTopRight.y) - Ot.Var.Ops.originOf(circularArray(contour, idx - 2).y)) > 30 &&
					// 	abs(Ot.Var.Ops.originOf(verticalBottomRight.x) - Ot.Var.Ops.originOf(circularArray(contour, idx - 2).x)) < 10
					// ) {
					// 	const deltaM0 = Ot.Var.Ops.originOf(verticalTopRight.y) - Ot.Var.Ops.originOf(verticalBottomRight.y);
					// 	const deltaM1 = Ot.Var.Ops.evaluate(verticalTopRight.y, instanceShsWghtMax) - Ot.Var.Ops.evaluate(verticalBottomRight.y, instanceShsWghtMax);
					// 	const diffM0 = deltaM0 < 30 ? 30 - deltaM0 : 0;
					// 	const diffM1 = deltaM1 < 80 ? 80 - deltaM1 : 0;
					...
					// 		y: makeVariance(
					// 			Ot.Var.Ops.originOf(verticalBottomRight.y) - diffM0,
					// 			Ot.Var.Ops.evaluate(verticalBottomRight.y, instanceShsWghtMax) - diffM1
					// 		),
							...
							if (
								...
								if (
									...
									newContour[topRightIdx] = {
										x: verticalTopRight.x,
										y: makeVariance(
											Ot.Var.Ops.originOf(horizontalTopRight.y) - 7,
											Ot.Var.Ops.evaluate(horizontalTopRight.y, instanceShsWghtMax) - 30
										),
										kind: 0,
									};
									newContour[topLeftIdx] = {
										x: verticalTopLeft.x,
										y: makeVariance(
											Ot.Var.Ops.originOf(horizontalTopLeft.y) - 7,
											Ot.Var.Ops.evaluate(horizontalTopLeft.y, instanceShsWghtMax) - 30
										),
										kind: 0,
									};
									// extended = true;
						...
						for (let ctrIdx = 0; ctrIdx < ctr.length; ctrIdx++) {
							...
							if (
								...
								if (
									...
									newContour[topLeftIdx] = {
										x: makeVariance(
											Ot.Var.Ops.originOf(verticalBottomLeft.x) + 1,
											Ot.Var.Ops.evaluate(verticalBottomLeft.x, instanceShsWghtMax) + 2
										),
										y: horizontalTopLeft.y,
										kind: 0,
									};
