...
function postProcess(font, references) {
	...
	function findBottomRight(contour) {
		for (let i = 0; i < contour.length; i++) {
			if (
				contour[i].kind === 0 && circularArray(contour, i + 1).kind === 1 && circularArray(contour, i + 2).kind === 2 && circularArray(contour, i + 3).kind === 0 && circularArray(contour, i + 4).kind === 1 && circularArray(contour, i + 5).kind === 2 && circularArray(contour, i + 6).kind === 0 &&
				distanceLight(circularArray(contour, i), circularArray(contour, i + 6)) < 60 &&
				distanceHeavy(circularArray(contour, i), circularArray(contour, i + 6)) < 160 &&
				...
				originLight(circularArray(contour, i + 6).x) < originLight(circularArray(contour, i + 3).x) &&
				originLight(circularArray(contour, i + 6).y) > originLight(circularArray(contour, i + 3).y)
			) {
				return i;
			}
		}
		...
		function fixLeftFallingBezier(ref) {
			...
			if ("leftFallingTopLeft" in ref) {
				...
				// if (inside(geoLeftP1Light, horizontalPolyLight) === false) {
					...
					let horizontalCurve2Light = new Bezier(h4xL + hXL,h4yL + hYL,h5xL + hXL,h5yL + hYL,h6xL + hXL,h6yL + hYL,h7xL + hXL,h7yL + hYL);
					...
					let horizontalCurve2Heavy = new Bezier(h4xH + hXH, h4yH + hYH, h5xH + hXH, h5yH + hYH, h6xH + hXH, h6yH + hYH, h7xH + hXH, h7yH + hYH);
